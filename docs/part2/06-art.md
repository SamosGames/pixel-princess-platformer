...
| `gen/` | AI-generated source PNGs + exact `<name>.prompt.txt` (model, cost, terms) + `PROMPTS.md` |
| `proc/` | Deterministic processing output for review: looped strips, Anna cells, composites, `report.json` |
| `pipeline-demo.png` | Raw generated figure → keyed → 32×48 cell, and raw mid segment → processed strip |
...
the catalogue (Yandex requirement 3.6 in `00-market.md`). The AI background pipeline makes
...
One generated landscape image becomes roughly one 1920-runtime-px segment after processing (§4.4).
...
decals). The generated kit sheet (§4) is reduced to that kit, then the generator expands it
procedurally:
...
| --- | --- | --- | --- |
| Light pools, moon, windows | Baked into background layers (the image model renders them; the quantizer keeps them) | 0 | **Yes** |
...
## 4. Pipeline: AI-generated source, deterministic `tools/gen` as the build

Decided by the human: art is generated by an AI image model (commercial-use licence recorded per file) and
committed as source.

> **Not tested — pending real generations.** Everything in §4 is design. No generated image had
...
for **every integer upscale and runtime draw**, but reduces raw generated images differently. Each raw pixel is **snapped to the palette first**,
...
### 4.0 Procedural vs AI-generated vs hybrid — for the human to choose in `review.html`

| | Procedural (code-painted on `px.mjs`, as in the mockups) | AI-sourced (processed by §4.2) | **Hybrid: AI backgrounds + procedural sprites/tiles/UI** |
| --- | --- | --- | --- |
...
| Determinism | Full, generator only. | Build deterministic; generation isn't (source committed). | Same as AI-sourced, backgrounds only. |
...
variance. Generate the AI Anna reference sheet anyway, as the design reference the
procedural painter follows. Switch characters to AI-sourced only if the Phase 0 side-by-side
in `review.html` shows processed AI sprites clearly beating the code painter. The choice
is the human's.
...
**Prototype evidence (throwaway code outside the repo; NOT a generation test).** Input was the real
...
distance. What it does **not** prove: real generated seams, horizon drift, or character drift (since measured in §4.10).
Those remain **not tested — pending real generations**.

Run twice, diff zero bytes: a CI check. `assets/` stays 100% generated, and the
"never hand-edit `assets/`" rule stays honest. Raw generated PNGs plus their prompts are the
...
- **Characters**: "full body, side-by-side grid of N poses, feet on a thin horizontal black
  ...
  sheet".
- **Sending rules**: only art prompts go to the image model; never repo code, secrets or private files.
...
### 4.6 Animation: what comes from the image model vs procedural

An image model can't hold frame-to-frame registration across 8 run frames. It generates **key poses**;
the build and runtime make the motion.

| Animation | From the image model (key poses, one grid image each) | Procedural (build or runtime) |
| --- | --- | --- |
...
- **Only commercial-use licences.** The model and its terms are recorded in every
  `<name>.prompt.txt`. The run in §4.10 used Google Gemini via OpenRouter: the Gemini API
  Additional Terms say Google won't claim ownership of generated content, and similar content
  may be generated for others. Any other provider or open-weights model needs its own recorded
  licence before use (e.g. FLUX.1-schnell weights are Apache-2.0). Non-commercial weights or
  outputs (FLUX.1-dev, CC-BY-NC, unverified hosted models) are rejected. Re-read the wording
  before production.
...
Measured in §4.10: **≈$0.067 and ≈12 s per image** with `google/gemini-3.1-flash-image` via
OpenRouter (the direct Gemini API key hit its quota). Prices and quotas change, so re-measure
before a large run. Every image still gets a human-or-validator acceptance check.
...
`npm run gen` stays offline and deterministic: it never calls an image model. Generation is a separate,
...
richness the AI-generated pipeline delivers (compare `proc/ai-composite-level1.png`). Jump/fall
poses read close to idle. Nothing is animated.
...
| --- | --- | --- | --- | --- |
| 0 | **AI pipeline proof** | Done: World 1 layers + Anna via the image API, processing prototype, `pipeline-demo.png`, composites, `review.html` (§4.10). Remaining: silhouette-separation lane metric and a sprite rim fix, face-feature overlay, real-iPhone density test | M | **Human approves `review.html`**; seams invisible at 1:1; lane gate passes for Anna; processing twice → identical bytes (already true) |
...
Phase 0 is the hard gate: it decides full ×2 vs fallback (c) and whether the AI-generated
pipeline meets the bar.
...
with `ao preview docs/part2/art/review.html`. Built and verified in headless Chrome (0 broken
images, 0 HTTP errors). Only the boss and menu AI columns are placeholders (not generated in
this run). Sections:

1. **Three columns per scene**: Part 1 current / procedural code mockup / AI-sourced
   composite, for Level 1 desktop, Level 1 iPhone landscape, the boss arena and the menu, plus
   the lane-contrast readout from `proc/report.json`.
2. **Live parallax preview**: the processed looping strips animated with CSS at 0.2/0.5/0.8
   speeds (disabled under `prefers-reduced-motion`).
3. **Backgrounds**: each raw generated segment with its accept/reject reason, the outpaint
   canvas, and the processed strips in horizontal-scroll containers.
4. **Anna**: reference sheet, idle and run, raw vs processed 32×48 cells (×4).
5. **Pipeline demo**: `pipeline-demo.png` (raw → keyed → quantize-then-vote cell; raw segment →
   processed strip). The wardrobe look-edit demo waits for the next run.
