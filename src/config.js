...
// Skins layer on top of these base looks as levels are cleared.
//
// The TEXT lives in src/i18n/*.js, not here: only the keys do. Explicit `*Key` fields rather than
// reusing `name`, so a call site that forgets to translate renders the literal key on screen (and
// in the tests) instead of quietly showing Italian to an English player. `id`, `sprite` and
// `color` are untouched — they are localStorage values and asset names, not prose.
export const CHARACTERS = [
  {
    id: "anna",
    nameKey: "char.anna.name",
    taglineKey: "char.anna.tagline",
    // Romantic one-liner shown on the selection card (keep it short + bracket-free — k.text
    // crashes on its own style tokens). Edit it in src/i18n/it.js, en.js and ru.js.
    descKey: "char.anna.desc",
    sprite: "anna",
    color: PALETTE.sky,
  },
  {
    ...
    id: "sognatrice",
    nameKey: "char.sognatrice.name",
    taglineKey: "char.sognatrice.tagline",
    descKey: "char.sognatrice.desc",
    sprite: "sognatrice",
    color: [240, 198, 116],
  },
  {
    ...
    id: "avventuriera",
    nameKey: "char.avventuriera.name",
    taglineKey: "char.avventuriera.tagline",
    descKey: "char.avventuriera.desc",
    sprite: "avventuriera",
    color: [196, 122, 88],
  },
];
...
// `key` is the sprite asset key (never translated); `nameKey` is the i18n key of the label the
// level-cleared reward card shows.
export const SKINS = [
  { key: "skirt", nameKey: "skin.skirt", afterLevel: 1 },
  { key: "bodice", nameKey: "skin.bodice", afterLevel: 2 },
  { key: "necklace", nameKey: "skin.necklace", afterLevel: 3 },
  { key: "crown", nameKey: "skin.crown", afterLevel: 4 },
  { key: "gloves", nameKey: "skin.gloves", afterLevel: 5 },
  { key: "cape", nameKey: "skin.cape", afterLevel: 6 },
];
...
// The finale text (heroineTitle / title / the seven-line letter) used to be a FINALE const
// here. It is prose, so it moved to the dictionaries with everything else: `finale.heroineTitle`,
// `finale.title` and `finale.message` in src/i18n/it.js, en.js and ru.js. Edit it there — it is the
// emotional centrepiece of the gift; keep it bracket-free and stick to widely-shipped emoji.
