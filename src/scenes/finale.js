...
// centred box showing a personalized message (edit finale.* in src/i18n/it.js, en.js and ru.js). A button
// (or Enter/Esc) returns to the menu.

import { k, setFrameCap } from "../kaplayCtx.js";
import { GAME_W, GAME_H, PALETTE, CHARACTERS, SKINS, PERF } from "../config.js";
...
export function registerFinaleScene() {
  k.scene("finale", () => {
    ...
    k.add([
      k.text(t("finale.finalTime", { time: formatDuration(getRunTime()) }), {
        size: 24,
        font: uiFont(t("finale.finalTime", { time: formatDuration(getRunTime()) })),
      }),
      k.pos(GAME_W / 2, 44),
      k.anchor("center"),
    ...
    k.add([
      k.text(t("finale.heroineTitle"), { size: 34, font: uiFont(t("finale.heroineTitle")) }),
      k.pos(GAME_W / 2, 132),
      k.anchor("center"),
    ...
    k.add([
      k.text(t("finale.title"), { size: 32, font: uiFont(t("finale.title")) }),
    ...
    k.add([
      ...
      k.text(t("finale.message"), { size: 22, width: boxW - 80, align: "center", lineSpacing: 7, font: "sans-serif" }),
    ...
    btn.add([
      k.text(t("finale.backToMenu"), { size: 24, font: uiFont(t("finale.backToMenu")) }),
      k.anchor("center"),
      k.color(...PALETTE.deepBlue),
    ]);
    btn.onHover(() => {
    ...
    lbBtn.add([
      k.text(t("finale.leaderboard"), { size: 22, font: "sans-serif" }),
      k.anchor("center"),
      k.color(...PALETTE.deepBlue),
    ]);
    lbBtn.onHover(() => {
