...
import { gameplayStart, gameplayStop, showFullscreen, trackEvent } from "../platform/yandex.js";
...
export function registerGameScene() {
  k.scene("game", () => {
    ...
    trackEvent("level_start", { level, world: def.nameKey, character: char.id });
    ...
    function die() {
      ...
      trackEvent("player_death", { level, livesLeft: left, checkpoint: !!checkpointAt });
      if (left <= 0) {
        // Out of lives: keep the current run intact until the player chooses. A rewarded video
        // can resume from the checkpoint; the normal button performs the full arcade reset.
        showGameOver(() => {
          resetRun();
    ...
    player.onCollide("key", (key) => {
      ...
      k.destroy(key);
      keyTaken = true;
      showBossHint(t("game.ballroomOpen"));
    });
    ...
    player.onCollide("checkpoint", (flag) => {
      ...
      trackEvent("checkpoint_reached", { level });
    ...
    k.add([k.text(t(char.nameKey), { size: 28, font: uiFont(t(char.nameKey)) }), k.pos(88, 18), k.color(...hudColor), k.fixed(), k.z(50)]);
    k.add([
      k.text(t(def.nameKey), { size: 20, font: uiFont(t(def.nameKey)) }),
      k.pos(88, 52),
      k.color(...hudColor),
      k.opacity(0.85),
      k.fixed(),
    ...
    const goalLabel = k.add([
      k.text(t("hud.goal"), { size: 18, font: "sans-serif" }),
      k.pos(88, 208),
      k.color(...hudColor),
      k.fixed(),
      k.z(50),
    ...
    k.onUpdate(() => {
      ...
      if (active) {
        ...
        if (sec !== lastInvSec) {
          invLabel.text = t("hud.invincible", { sec });
          lastInvSec = sec;
        }
      } else lastInvSec = -1;
      ...
      if (feather) {
        ...
        if (sec !== lastFeatherSec) {
          featherLabel.text = t("hud.feather", { sec });
          lastFeatherSec = sec;
        }
      } else {
        lastFeatherSec = -1;
    ...
    player.onCollide("collectible", (item) => {
      ...
      trackEvent("collectible_collected", { level, count: collected, total: collectiblesTotal });
      if (collected >= collectiblesTotal) {
        goalLabel.text = t("hud.goalPortal");
        showBossHint(t("game.allCollected"));
      }
    ...
    player.onCollide("heart", (h) => {
      ...
      updateLives();
      trackEvent("heart_collected", { level, lives: getLives() });
    });
    ...
    function showBossHint(msg) {
      ...
      const hint = k.add([
        k.text(msg, { size: 24, font: uiFont(msg) }),
    ...
    player.onCollide("goal", () => {
      ...
      if (bossLevel) {
        if (k.get("boss").length > 0) {
          showBossHint(t("game.bossDefeat"));
          return;
        }
        if (!keyTaken) {
          showBossHint(t("game.bossKey"));
          return;
        }
      }
      finished = true;
      ...
      const stars = collectiblesTotal > 0
        ? collected === collectiblesTotal
          ? 3
          : collected >= Math.ceil(collectiblesTotal / 2)
            ? 2
            : 1
        : 1;
      recordLevelResult(level, stars, getRunTime());
      trackEvent("level_complete", { level, stars, collectibles: collected, total: collectiblesTotal, score: getScore() });
      setCurrentLevel(level + 1); // persist progress (drives the skin layering next level)
      const next = () => showReward(reward, collected, collectiblesTotal, icon, level + 1, stars);
      if ([2, 4, 6].includes(level)) void showFullscreen().finally(next);
      else next();
    });

    // --- Chapter banner that fades after a moment ---
    const banner = k.add([
      k.text(t("game.chapter", { n: level, name: t(def.nameKey) }), {
        size: 44,
        font: uiFont(t("game.chapter", { n: level, name: t(def.nameKey) })),
      }),
      k.pos(GAME_W / 2, 110),
      k.anchor("center"),
      k.color(...hudColor),
    ...
    // A short control prompt closes the first-30-seconds gap: the goal is visible in the HUD,
    // and the input scheme is shown once without taking space from the level itself.
    const controlsHint = k.add([
      k.text(coarsePointer ? t("hud.controlsTouch") : t("hud.controlsKeyboard"), { size: 20, font: "sans-serif" }),
      k.pos(GAME_W / 2, GAME_H - 180),
      k.anchor("center"),
      k.color(...hudColor),
      k.opacity(0.92),
      k.fixed(),
    ...
    let controlsHintT = 0;
    controlsHint.onUpdate(() => {
      controlsHintT += k.dt();
      if (controlsHintT > 5) controlsHint.opacity = Math.max(0, controlsHint.opacity - k.dt() * 0.8);
      if (controlsHint.opacity <= 0) k.destroy(controlsHint);
    });
...
function showReward(reward, got, total, icon, nextLevel, stars) {
  ...
  k.add([
    k.text(t("reward.title"), { size: 50, font: uiFont(t("reward.title")) }),
  ...
  if (reward) {
    ...
    k.add([
      k.text(t("reward.unlocked"), { size: 26, font: uiFont(t("reward.unlocked")) }),
    ...
    k.add([
      k.text(t(reward.nameKey), { size: 38, font: uiFont(t(reward.nameKey)) }),
  ...
  k.add([
    ...
    k.text(`${icon} ${got}/${total}    ${t("reward.stars", { stars })}    ★ ${getScore()}    ⏱ ${formatDuration(getRunTime())}`, { size: 26, font: "sans-serif" }),
  ...
  const label = more ? t("reward.continue") : toFinale ? t("reward.toBall") : t("reward.toMenu");
  ...
  btn.add([
    k.text(label, { size: 26, font: uiFont(label) }),
    k.anchor("center"),
    k.color(...PALETTE.deepBlue),
  ]);
  ...
  if (toFinale) {
    k.add([
      k.text(t("reward.storyWaits"), { size: 18, font: uiFont(t("reward.storyWaits")) }),
