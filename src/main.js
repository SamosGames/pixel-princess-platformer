...
import { getRunTime, hydrateCloudProgress } from "./state.js";
import { initI18n, onLangChange } from "./i18n/index.js";
import { initYandex, whenYandexReady, trackEvent } from "./platform/yandex.js";
import { bindTouchButtons, getInput } from "./controls.js";
import { bindAudioToggle, paintAudioToggle } from "./ui/audioToggle.js";
...
// Keep browser gestures and selection from escaping the game on desktop or mobile.
document.addEventListener("contextmenu", (event) => event.preventDefault());
document.addEventListener("selectstart", (event) => event.preventDefault());

// Resolve the language BEFORE anything draws or binds: ?lang= → saved choice → the device's
// own language (with Russian, English and Italian dictionaries). It also paints every data-i18n
// node in index.html, so the first frame is already in the right language rather than flashing
// Italian at a Russian or English player. See src/i18n/index.js.
initI18n();

// The same static source can be hosted on Vercel, Yandex Games, or localhost. Keep previews
// useful on the fallback host, but share the actual page where the player is running it.
const pageUrl = new URL(document.URL);
document.querySelector('meta[property="og:url"]')?.setAttribute("content", pageUrl.href);

// Let the loading scene wait for the platform handshake and cloud save before building the menu.
// Local development resolves both promises immediately, while Yandex gets the correct language
// and cross-device progress before the first canvas scene is constructed.
const platformReady = initYandex();
const cloudProgressReady = hydrateCloudProgress();
const bootReady = Promise.all([platformReady, cloudProgressReady, whenYandexReady()]);
void platformReady.then(() => trackEvent("session_start", { surface: "game" }));

// The audio button's aria-label is on/off state, not static text, so no data-i18n attribute can
// reach it — it repaints itself whenever the language changes.
onLangChange(paintAudioToggle);
...
registerLoadingScene(bootReady);
registerMenuScene();
registerGameScene();
registerFinaleScene();
