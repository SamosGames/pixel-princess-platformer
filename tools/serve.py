...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        # Yandex serves the real SDK at /sdk.js inside its platform shell. Local smoke tests
        # still load the production index, so return a tiny successful stub instead of a noisy
        # 404; the adapter sees no YaGames global and stays on its local fallback.
        if urlsplit(self.path).path == "/sdk.js":
            body = b"// Local Yandex Games SDK stub.\n"
