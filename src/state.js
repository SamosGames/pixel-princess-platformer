...
import { loadCloudProgress, saveCloudProgress } from "./platform/yandex.js";
...
const KEYS = {
  ...
  levelStars: "pj.levelStars", // JSON map of best stars earned per level
  bestTimes: "pj.bestTimes",   // JSON map of best completion times per level
};
...
const state = {
  ...
  levelStars: readJson(KEYS.levelStars, {}),
  bestTimes: readJson(KEYS.bestTimes, {}),
};
...
function progressSnapshot() {
  return {
    version: 1,
    heroine: state.selectedCharacter,
    unlockedLevel: state.currentLevel,
    score: state.score,
    lives: state.lives,
    checkpoint: getCheckpoint(),
    heartsTaken: getHeartsTaken(),
    levelStars: { ...state.levelStars },
    bestTimes: { ...state.bestTimes },
    settings: {
      music: read("pj.music"),
      sfx: read("pj.sfx"),
      musicVol: read("pj.musicVol"),
      sfxVol: read("pj.sfxVol"),
      language: read("pj.lang"),
    },
  };
}
...
function scheduleCloudSave() {
  ...
  cloudSaveTimer = setTimeout(() => {
    cloudSaveTimer = null;
    void saveCloudProgress(progressSnapshot());
  }, 250);
}

function persistMaps() {
  write(KEYS.levelStars, JSON.stringify(state.levelStars));
  write(KEYS.bestTimes, JSON.stringify(state.bestTimes));
}

function mergeCloudProgress(data) {
  if (!data || typeof data !== "object") return;
  if (typeof data.heroine === "string") state.selectedCharacter ||= data.heroine;
  if (Number.isFinite(Number(data.unlockedLevel))) {
    state.currentLevel = Math.max(state.currentLevel, clampLevel(Number(data.unlockedLevel)));
  }
  if (Number.isFinite(Number(data.score))) state.score = Math.max(state.score, clampScore(Number(data.score)));
  if (Number.isFinite(Number(data.lives))) state.lives = Math.max(state.lives, clampLives(Number(data.lives)));

  for (const [level, stars] of Object.entries(data.levelStars || {})) {
    if (Number.isFinite(Number(stars))) {
      state.levelStars[level] = Math.max(Number(state.levelStars[level]) || 0, Math.min(3, Number(stars)));
    }
  }
  for (const [level, time] of Object.entries(data.bestTimes || {})) {
    if (Number.isFinite(Number(time)) && Number(time) > 0) {
      const previous = Number(state.bestTimes[level]);
      state.bestTimes[level] = previous > 0 ? Math.min(previous, Number(time)) : Number(time);
    }
  }
  if (state.selectedCharacter) write(KEYS.character, state.selectedCharacter);
  write(KEYS.level, String(state.currentLevel));
  write(KEYS.score, String(state.score));
  write(KEYS.lives, String(state.lives));
  if (data.checkpoint && Number(data.checkpoint.level) === state.currentLevel) {
    setCheckpoint(data.checkpoint);
  }
  if (Array.isArray(data.heartsTaken)) {
    const hearts = [...new Set([...getHeartsTaken(), ...data.heartsTaken.filter(Number.isFinite)])];
    write(KEYS.heartsTaken, JSON.stringify(hearts));
  }
  persistMaps();
}

/** Load the anonymous/authorized Yandex save before the first menu is built. */
...
export function recordLevelResult(level, stars, timeMs) {
  const key = String(level);
  const safeStars = Math.max(0, Math.min(3, Math.floor(Number(stars) || 0)));
  const safeTime = Math.max(0, Math.round(Number(timeMs) || 0));
  state.levelStars[key] = Math.max(Number(state.levelStars[key]) || 0, safeStars);
  if (safeTime > 0) {
    const previous = Number(state.bestTimes[key]);
    state.bestTimes[key] = previous > 0 ? Math.min(previous, safeTime) : safeTime;
  }
  persistMaps();
  scheduleCloudSave();
}
...
export function resetProgress() {
  ...
  try {
    ...
    window.localStorage.removeItem(KEYS.checkpoint);
    window.localStorage.removeItem(KEYS.heartsTaken);
    window.localStorage.removeItem(KEYS.levelStars);
    window.localStorage.removeItem(KEYS.bestTimes);
  } catch {
    // no-op
  }
  scheduleCloudSave();
