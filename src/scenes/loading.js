...
import { getPlatformLanguage, markGameReady } from "../platform/yandex.js";
import { setLang, t, uiFont } from "../i18n/index.js";
import { setInputEnabled } from "../controls.js";

export function registerLoadingScene(bootReady = Promise.resolve()) {
  k.scene("loading", () => {
    ...
    k.add([
      k.text(t("brand.title"), { size: 44, font: uiFont(t("brand.title")) }),
    ...
    k.add([
      k.text(t("loading.text"), { size: 28, font: uiFont(t("loading.text")) }),
    ...
    k.onLoad(() => {
      Promise.resolve(bootReady).then(() => {
        const platformLanguage = getPlatformLanguage();
        if (platformLanguage) setLang(platformLanguage);
        markGameReady();
