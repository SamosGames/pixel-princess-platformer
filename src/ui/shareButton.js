...
/**
 * The canonical link. Prefer an absolute og:url when a host supplies one; otherwise use the
 * current page. The Yandex archive has no fixed public URL at build time.
 */
export function gameUrl() {
  const meta = document.querySelector('meta[property="og:url"]')?.content;
  const pageUrl = new URL(document.URL);
  try {
    return meta?.trim() ? new URL(meta, pageUrl.href).href : pageUrl.href;
  } catch {
    return pageUrl.href;
  }
}
...
async function share() {
  ...
  if (navigator.share) {
    try {
      await navigator.share({ title: t("brand.title"), text: t("share.text"), url });
      return;
    } catch (err) {
  ...
  const payload = `${t("share.text")} ${url}`;
  try {
  ...
  flash(legacyCopy(payload) ? t("share.copied") : url);
}
