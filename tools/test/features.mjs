...
import { launchBrowser, routeVendorKaplay, PAGE_LOCALE } from "./browser.mjs";
...
try {
  const page = await browser.newPage({ viewport: { width: 1280, height: 720 }, locale: PAGE_LOCALE });
  ...
  const menuJourney = await page.evaluate(() =>
    window.__pj.k.get("*", { recursive: true }).map((o) => o.text).filter((text) => typeof text === "string"),
  );
  check("menu shows the journey map", menuJourney.some((text) => /IL TUO VIAGGIO|YOUR JOURNEY|ТВОЙ МАРШРУТ/.test(text)));
  ...
  await page.evaluate(() => {
    window.__pjEvents = [];
    window.addEventListener("pj:analytics", (event) => window.__pjEvents.push(event.detail));
  });
  ...
  const gameBriefing = await page.evaluate(() =>
    window.__pj.k.get("*", { recursive: true }).map((o) => o.text).filter((text) => typeof text === "string"),
  );
  check("game shows a clear objective", gameBriefing.some((text) => /Obiettivo:|Goal:|Цель:/.test(text)));
  check(
    "analytics emits level_start",
    await page.evaluate(() => window.__pjEvents?.some((event) => event.name === "level_start" && event.params.level === 2)),
  );
  ...
  const gameOverAdHidden = await page.$eval("#gameover-ad-btn", (el) => el.hidden);
  check("Game Over ad is Yandex-only", gameOverAdHidden);

  // The regular Game Over button performs the full reset. A rewarded video, when available on
  // Yandex, takes the separate checkpoint path and is not exercised by the local fallback.
