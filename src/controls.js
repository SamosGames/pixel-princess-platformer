...
/** Keep keyboard/touch input inert until the platform has reported Game Ready. */
export function setInputEnabled(enabled) {
  inputEnabled = Boolean(enabled);
  if (!inputEnabled) resetInput();
}
...
export function bindKeyboard() {
  k.onKeyDown("left", () => {
    if (inputEnabled) input.left = true;
  });
  k.onKeyDown("right", () => {
    if (inputEnabled) input.right = true;
  });
  ...
  k.onKeyPress(["space", "up"], () => {
    if (inputEnabled) input.jump = true;
  });
  k.onKeyDown(["space", "up"], () => {
    if (inputEnabled) input.jumpHeld = true;
  });
